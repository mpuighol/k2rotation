...
for i in range(len(stash)):
    ...
    medflux = pd.rolling_median(data['flux'][ok], 500, center=True)
...
#     scores = model.score(periods)
    model = LombScargle(data['time'][ok][ok2], data['flux'][ok][ok2]- medflux[ok2], data['error'][ok][ok2])
